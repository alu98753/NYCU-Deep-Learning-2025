...
class Dataset_Dance(torchData):
    def __init__(self, root, transform, mode='test', video_len=7, partial=1.0):
        ...
        # data_num = len(glob('../LAB4_Dataset/testtest/*'))
        data_num = 5
...
class Test_model(VAE_Model):
    ...
    def val_one_step(self, img, label, idx=0):
        ...
        decoded_frame_list = [img[0].cpu()]  # first frame作為第一幀輸入 shape: (1, 3, 32, 64)
        label_list = []

        # TODO[x]
        for t in range(1, 630):
            prev_frame = decoded_frame_list[t-1].to(self.args.device)
            frame_feature  = self.frame_transformation(prev_frame)
            label_feature  = self.label_transformation(label[t].to(self.args.device))
            z, _, _ = self.Gaussian_Predictor(frame_feature, label_feature)
            z = torch.randn_like(z)
            fused = self.Decoder_Fusion(frame_feature, label_feature , z)
            pred = self.Generator(fused)
            decoded_frame_list.append(pred.cpu())
            
            label_list.append(label_feature.cpu()) 
            '''
            label_feature : 儲存的是壓縮後的 label latent 特徵	可以分析或比較 latent 特徵的變化與模型輸出分數
            label[t]	儲存的是原始 pose image，方便做 GIF 視覺對照	
            ''' 
        ...
        # Reshape the generated frame to (630, 3 * 64 * 32)     # 展平成 (630, 6144) 給後續儲存/比對用
