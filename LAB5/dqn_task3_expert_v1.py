...
class DQNAgent:         # task1 CartPole-v1  # task2 ALE/Pong-v5
    def __init__(self, env_name="ALE/Pong-v5", args=None): 
        ...
        if args.expert_load_path:
            ...
            n = min(len(expert_data), args.expert_init_limit)
            for i in range(n):
                s, a, r, s_next, done = expert_data[i]
                # 這裡用 abs(r) 當作初始 td-error（或者自己定義）
                td_err = abs(r)+ 1e-6
                self.memory.add((s, a, r, s_next, done), td_err)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--expert-init-limit", type=int, default=40000,help="Max number of expert transitions to preload")
