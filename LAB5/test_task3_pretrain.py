...
class DQNAgent:         # task1 CartPole-v1  # task2 ALE/Pong-v5
    def __init__(self, env_name="ALE/Pong-v5", args=None): 
        ...
        if args.expert_load_path:
            ...
            k = min(len(expert_data), args.expert_init_limit)
            self.pretrain_from_expert(expert_data[:k], epochs=200, batch_size=1024, save_every=10)
    ...
    # def pretrain_from_expert(self, expert_data, epochs=5, batch_size=1024, save_every=5):
    #     """
    #     用专家轨迹做 BC 预训练，分批加载以避免 OOM，
    #     每 save_every 个 epoch 保存一次 q_net 权重。
    #     """
    #     # 1. 把 state/action 拆出来
    #     states_np  = np.stack([d[0] for d in expert_data], axis=0)
    #     actions_np = np.array([d[1] for d in expert_data], dtype=np.int64)

    #     ds = TensorDataset(torch.from_numpy(states_np), torch.from_numpy(actions_np))
    #     loader = DataLoader(ds, batch_size=batch_size, shuffle=True, drop_last=True)
    #     criterion = nn.CrossEntropyLoss()

    #     print(f"🛠️ BC pretrain: {len(ds)} samples, epochs={epochs}, batch={batch_size}")
    ...
    #             logits = self.q_net(batch_states)
    ...
    #         print(f"  BC Epoch {ep}/{epochs}, avg loss {avg_loss:.4f}")

    #         # —— 新增：每 save_every 个 epoch 就存一次模型
    #         if ep % save_every == 0:
    ...
    #     # 同步 target network
    #     self.target_net.load_state_dict(self.q_net.state_dict())
    #     print("✅ BC pretrain done, target_net synced.")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--expert-init-limit", type=int, default=30000,help="Max number of expert transitions to preload")
