        ...
        with torch.no_grad():
            ...
            for step in range(self.total_iter):
                if step == self.sweet_spot:
                    break
                ratio = (step+1) / self.sweet_spot #this should be updated
    
                z_indices_predict, mask_bc = self.model.inpainting(z_indices_predict, self.mask_func, mask_num, mask_bc, ratio)
...
#TODO3 step1-2: modify the path, MVTM parameters
    parser.add_argument('--load-transformer-ckpt-path', type=str, default='transformer_checkpoints_1e4/best_val.pth', help='load ckpt')
    ...
    #MVTM parameter
    parser.add_argument('--sweet-spot', type=int, default=10, help='sweet spot: the best step in total iteration 表中途可提前終止（解碼到某輪即可）')
